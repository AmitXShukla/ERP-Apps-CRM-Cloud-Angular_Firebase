...
export class OpportunityComponent implements OnInit, OnDestroy {
  ...
  getData(formData?) {
    ...
    this.querySubscription = this._backendService.getDocs('OPPURTUNITY', formData).subscribe((res) => {
      if (res.length > 0) {
  ...
  setData(formData) {
    this.dataLoading = true;
    return this._backendService.setDoc('OPPURTUNITY', formData, this.authState.uid).then(res => {
      if (res) {
  ...
  updateData(formData) {
    this.dataLoading = true;
    this.querySubscription = this._backendService.updateDoc('OPPURTUNITY', formData._id, formData).then(res => {
      if (res) {
        this.savedChanges = true;
        this.error = false;
        this.errorMessage = "";
  ...
  getDoc(docId) {
    ...
    this.dataLoading = true;
    this.data$ = this._backendService.getDoc('OPPURTUNITY', docId).subscribe(res => {
      if (res) {
        this.data$ = res;
        this.editDataForm = this._fb.group({
          _id: ['', Validators.required],
  ...
  deleteDoc(docId) {
    if (confirm("Are you sure want to delete this record ?")) {
      this.dataLoading = true;
      this._backendService.deleteDoc('OPPURTUNITY', docId).then(res => {
        if (res) {
          this.error = false;
          this.errorMessage = "";
          this.dataLoading = false;
